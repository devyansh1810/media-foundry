...
import { JobProgress, JobResult, FilterOptions } from '@/types'
...
export default function FiltersPage() {
  ...
  const handleProcess = async () => {
    ...
    const options: FilterOptions = {
      filters: [
        { type: 'scale', width: parseInt(width), height: parseInt(height) },
      ],
    }
    setProcessing(true)
    ...
    try {
      await service.processJob('filter', file, options, setProgress, (res) => {
        setResult(res)
        setProcessing(false)
      })
    } catch (error) {
  ...
  const handleDownload = () => {
    ...
    a.href = url
    a.download = `filtered-${file?.name || 'output.mp4'}`
    a.click()
  ...
  return (
    <div className="container px-4 py-16">
      <div className="mx-auto max-w-2xl">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Sliders className="h-6 w-6" />Video Filters</CardTitle>
            <CardDescription>Apply scale filter to your video</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            {file && !processing && !result && (
              <>
                ...
                <Button onClick={handleProcess} className="w-full" size="lg">Apply Filter</Button>
              </>
            )}
